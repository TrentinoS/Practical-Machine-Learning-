...
inTrain  <- caret::createDataPartition(training$classe, p=0.7, list=FALSE)
...
NZV <- caret::nearZeroVar(TrainSet)
...
corrplot::corrplot(corMatrix, order = "FPC", method = "color", type = "lower", 
...
# model fit
set.seed(12345)
controlRF <- caret::trainControl(method="cv", number=3, verboseIter=FALSE)
modFitRandForest <- caret::train(classe ~ ., data=TrainSet, method="rf",
                          trControl=controlRF)
